...
DELAY_TIME  EQU 0x186A00	;Delay counter
...
loop
	...
	LDR R2, =DELAY_TIME	;Load R2 with DELAY_TIME
...
FINISH	
	...
	LDR 		R2, =DELAY_TIME ;Load R2 with DELAY_TIME
Inner_loop1	;inner loop for delay
	TEQ			R2, #0	; Test if R2 == 0, If yes then z=1, else z=0 (continue decrementing)
	BEQ if_else1 ;Branch to if_else when z==1
if_true1	;When R2 is not 0
	SUB			R2, R2, #1	;Decrement R2
	B			Inner_loop1 ;Loop back to Inner_loop1 and continue checking/decrementing R2	if needed
if_else1 ;Z==1 Condition
	B			FINISH1	;Branch to FINISH
FINISH1	

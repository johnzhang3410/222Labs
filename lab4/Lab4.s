...
GPIOA_MODER 	EQU 0x40020000	; Base address of GPIOA, Display
GPIOB_MODER		EQU	0x40020400	; Base address of GPIOB	
GPIOC_MODER		EQU	0x40020800	; Base address of GPIOC, LED
...
RAND_DATA	EQU 0x20000000 ;Reserve address for random number
	...
	STR	R0,[R1]		   ; store the random number in memory address RAND-DATA
...
BTN_FLAG	EQU 0x20000004 ;Reserve address for button flag
	...
	STR	R0,[R1]		   ; Store the button flag in memory address BTN_FLAG	
...
;----- Nested Vector Interrupt Controller (NVIC). The NVIC is part of the ARM core
	...
	; Enable clock for SYSCFG on Peripheral Bus No. 2, set 14th bit
	...
	;mask (disable) ERQ, no jump to ISR on button_press
...
;Masking ERQ and clear flag in stage 1 and 3 to prevent entering IRQ
;Set ERQ and flag at start of stage 2 to enter IRQ (will enter on button_press) 
	...
	;ERQ is masked and interrupt flag is cleared in setup
...
end1	
	...
	;stage 2
	;poll for interrupts every 1ms
	;button is pressed when btn_flag is set
	MOV R3, #0 ;Increments this counter until input received, convert + display this in stage 3

	;Unmask ERQ	
	LDR		R1,=EXTI ; Load EXTI_EMR Register
	LDR		R0,[R1, #EXTI_IMR] ; Load EXTI_IMR Register
	ORR		R0,R0,#0x2000 ;Set bit to unmask ERQ
	STR		R0,[R1, #EXTI_IMR] ; Write back EXTI_IMR Register
	
	;At this point, IRQ will be entered on button_press
	;Instead of polling to check if button_press, now we enter IRQ once button is pressed
	;poll to check if the btn_flag has been set to 1, if yes then button has been pressed
poll
	...
	LDR	R1,	=BTN_FLAG  ; load the address of the static variable
	LDR R4, [R1]	
	TEQ	R4, #1		;Check if the btn_flag has been set (button_pressed)   
...
;RGB is blue at this point due to button_press and visit to IRQ
;Mask ERQ so we don't enter IRQ when displaying result, Reset btn_flag
	;mask (disable) ERQ, no jump to ISR on button_press
	LDR		R1,=EXTI ; Load EXTI_EMR Register
	LDR		R0,[R1, #EXTI_IMR] ; Load EXTI_IMR Register
	AND		R0,R0,#0xFFFFDFFF ;Set bit to unmask ERQ, clear to mask ERQ
...
;stage 3
;convert counter to BCD and display it, then delay 5s before looping main
...
EXTI15_10_IRQHandler PROC
	...
	;Button has been pressed so we set the btn_flag to 1
	MOV R0,	#1 ; Flag: '1': Button Pressed; '0': Button serviced or never pressed 
	LDR	R1,	=BTN_FLAG  ; load the address of the static variable
	STR	R0,[R1]		   ; Store the button flag in memory address BTN_FLAG	

	;Button is pressed so RGB turn blue
	LDR	R1,=GPIOC_MODER
	MOV	R0, #0x3  ; RGB BLUE/ON
	STR	R0, [R1, #GPIO_ODR]	; Write back the data to output data register
	NOP	
				
	; Clear the interrupt flag as the last steo in ISR handling
	LDR	R1,=EXTI ; Load EXTI_EMR Register
